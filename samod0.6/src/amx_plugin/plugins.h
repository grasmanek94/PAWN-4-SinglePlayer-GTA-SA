...
struct DLLInfo
{
	...
	bool Init()
	{
		...
		if(this->IsAMX)
		{
			...
			xxFileToLoad.insert(0,(boost::filesystem::current_path() / "libraries" / "pluginsa" / "plugins").string());
		...
		else
		{
			xxFileToLoad.insert(0,this->Identifier);
			xxFileToLoad.insert(0,"\\");
			xxFileToLoad.insert(0,(boost::filesystem::current_path() / "libraries" ).string());
			this->Instance = LoadLibrary(xxFileToLoad.c_str());
		}
...
void LoadPlugins()
{
	...
	stream << (boost::filesystem::current_path() / "libraries" / "pluginsa" / "plugins" / "amx_plugins.ini").string().c_str();
	std::istringstream iss(get_file_contents(stream.str().c_str()));
	dbg(("[DBG]LoadPlugins()->get_file_contents(%s)->%s",stream.str().c_str(),iss.str().c_str()));
	std::string sub; 
	while (iss >> sub) 
	{
		LoadPlugin(sub,true);
	...
	stream.clear();
	stream << (boost::filesystem::current_path() / "libraries" /  "plugins.ini").string().c_str();
	std::istringstream iss2(get_file_contents(stream.str().c_str()));
	dbg(("[DBG]LoadPlugins()->get_file_contents(%s)->%s",stream.str().c_str(),iss2.str().c_str()));
	while (iss2 >> sub) 
	{
