...
ULONG myIDirect3DDevice9::Release(void)
{
	...
	SDX::OnRelease_After(count);
	//gl_myPresentationParams = NULL;
	return (count);
}
...
D3DPRESENT_PARAMETERS*   gl_myPresentationParams;

HRESULT myIDirect3DDevice9::Reset(D3DPRESENT_PARAMETERS* pPresentationParameters)
{	
	SDX::OnReset_Before(pPresentationParameters);
	gl_myPresentationParams = pPresentationParameters;
    HRESULT result = m_pIDirect3DDevice9->Reset(pPresentationParameters);
	SDX::OnReset_After(pPresentationParameters,result);
	return result;
}
...
HRESULT myIDirect3DDevice9::Present(CONST RECT* pSourceRect,CONST RECT* pDestRect,HWND hDestWindowOverride,CONST RGNDATA* pDirtyRegion)
{
    ...
    // ... draw own stuff ...
    SDX::OnPresent_Before(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion);
    // call original routine
	HRESULT hres = m_pIDirect3DDevice9->Present( pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion);
	SDX::OnPresent_After(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, hres);
	return (hres);
}
...
{
	SDX::OnBeginScene_Before();
	HRESULT result = m_pIDirect3DDevice9->BeginScene(); 
	SDX::OnBeginScene_After(result);
    return result;
}

HRESULT myIDirect3DDevice9::EndScene(void)
{
	SDX::OnEndScene_Before();
	HRESULT result = m_pIDirect3DDevice9->EndScene();
	SDX::OnEndScene_After(result);
    return result;
}
