...
CD3DRender				*render = new CD3DRender( 256 );
...
HRESULT CD3DFont::Print (  float x, float y, DWORD color, const char *szText, bool textcoloring)
{
	...
	static DWORD states[2] = {0,0};
	m_pD3Ddev->GetRenderState(D3DRS_CLIPPING,&states[0]);
	m_pD3Ddev->GetRenderState(D3DRS_ZENABLE,&states[1]);
	m_pD3Ddev->SetRenderState( D3DRS_CLIPPING, 0 );
	m_pD3Ddev->SetRenderState ( D3DRS_ZENABLE, 0 );
	...
	m_pD3Ddev->SetRenderState( D3DRS_CLIPPING, states[0] );
	m_pD3Ddev->SetRenderState ( D3DRS_ZENABLE, states[1] );
...
HRESULT CD3DFont::PrintShadow ( float x, float y, DWORD color, const char *szText, bool textcoloring)
{
	return Print( x, y, color, szText, textcoloring );
}

float CD3DFont::DrawLength ( const char *szText, bool textcoloring ) const
{
	float	len = 0.0f;
...
void CD3DRender::D3DBox ( float x, float y, float w, float h, D3DCOLOR color, bool clip )
{
	static DWORD states[2] = {0,0};
	m_pD3Ddev->GetRenderState(D3DRS_CLIPPING,&states[0]);
	m_pD3Ddev->GetRenderState(D3DRS_ZENABLE,&states[1]);
	m_pD3Ddev->SetRenderState( D3DRS_CLIPPING, clip );
	m_pD3Ddev->SetRenderState ( D3DRS_ZENABLE, clip );
	if ( SUCCEEDED(Begin(D3DPT_TRIANGLELIST)) )
	{
		D3DColor( color );
		D3DVertex2f( x, y );
...
void CD3DRender::D3DBoxi ( int x, int y, int w, int h, D3DCOLOR color, int maxW, bool clip )
{
	if ( maxW )
	{
		if ( w >= maxW )
			( w = maxW );
		D3DBox( (float)x, (float)y, (float)w, (float)h, color, clip );
	}
	else
	{
		D3DBox( (float)x, (float)y, (float)w, (float)h, color, clip );
...
void CD3DRender::D3DBoxBorder ( float x, float y, float w, float h, D3DCOLOR border_color, D3DCOLOR color, bool clip )
{
	D3DBox( x, y, w, 1.0f, border_color, clip );
	D3DBox( x + w, y, 1.0f, h, border_color, clip );
	D3DBox( x, y, 1.0f, h, border_color, clip );
	D3DBox( x, y + h, w, 1.0f, border_color, clip );
	D3DBox( x + 1.0f, y + 1.0f, w - 1.0f, h - 1.0f, color, clip );
}

void CD3DRender::D3DBoxBorderi ( int x, int y, int w, int h, D3DCOLOR border_color, D3DCOLOR color, bool clip )
{
	D3DBoxBorder( (float)x, (float)y, (float)w, (float)h, border_color, color, clip );
}

bool CD3DRender::DrawLine ( const D3DXVECTOR3 &a, const D3DXVECTOR3 &b, DWORD dwColor, bool clip )
{
	if ( FAILED(CD3DBaseRender::BeginRender()) )
	{
		return false;
	...
	m_pD3Ddev->SetTextureStageState( 0, D3DTSS_COLOROP, D3DTOP_MODULATE );
	m_pD3Ddev->SetTextureStageState( 0, D3DTSS_ALPHAOP, D3DTOP_MODULATE );
	m_pD3Ddev->SetRenderState ( D3DRS_ZENABLE, states[0] );
	m_pD3Ddev->SetRenderState( D3DRS_CLIPPING, states[1] );
...
bool CD3DRender::DrawTriangleInWorld ( const D3DXVECTOR3 &a, const D3DXVECTOR3 &b, const D3DXVECTOR3 &c, DWORD dwColor, bool clip )
{
	if ( FAILED(CD3DBaseRender::BeginRender()) )
	{
		return false;
	...
	m_pD3Dbuf->Lock( 0, 0, (void **) &lineList, D3DLOCK_DISCARD | D3DLOCK_NOSYSLOCK ); // flogs: D3DLOCK_NOSYSLOCK, D3DLOCK_DISCARD
	{
		...
		lineList[2].y = c.y;
		lineList[2].z = c.z;
		lineList[2].color = dwColor;
		lineList[2].specular = dwColor;
	}
	...
	m_pD3Ddev->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 1, lineList, sizeof(lineList) / 3 );
...
bool CD3DRender::DrawRectangleInWorld ( const D3DXVECTOR3 &a, const D3DXVECTOR3 &b, const D3DXVECTOR3 &c, const D3DXVECTOR3 &d, DWORD dwColor, bool clip )
{
	if ( FAILED(CD3DBaseRender::BeginRender()) )
	{
		return false;
	...
	m_pD3Dbuf->Lock( 0, 0, (void **) &lineList, D3DLOCK_DISCARD | D3DLOCK_NOSYSLOCK ); // flogs: D3DLOCK_NOSYSLOCK, D3DLOCK_DISCARD
	{
		...
		lineList[3].y = d.y;
		lineList[3].z = d.z;
		lineList[3].color = dwColor;
		lineList[3].specular = dwColor;
	}
	...
	m_pD3Ddev->DrawPrimitiveUP( D3DPT_TRIANGLESTRIP, 2, lineList, sizeof(lineList) / 4 );
