...
bool WriteTargetDataForPed ( CPedSAInterface * pPed, DWORD vecTargetPos, CVector * origin )
{
	...
	internal__samod::OnFireWeapon(pTargetingPed,origin);
    // local ped firing
...
void Event_PostFire ( void )
{
    ...
    //postfire
	internal__samod::OnPostFireWeapon(pTargetingPed);
    bWeaponFire = false;
}
    ...
    //if ( m_pBulletImpactHandler )
    {
        ...
        if ( pInitiator )
        {
            ...
            //if ( IsLocalPlayer ( pInitiator ) )
            {
                ...
                internal__samod::BulletImpactHandler ( pInitiator, pVictim, &vecLastLocalPlayerBulletStart, &vecLastLocalPlayerBulletEnd );
            }
        }
    }
}
