...
#endif
#ifndef MYIDIRECT3DDEVICE9
#define MYIDIRECT3DDEVICE9
...
namespace SDX
{
	#ifdef SDX_STATIC_LIB
	#define EXTERN__ extern
	#else
	#define EXTERN__ 
	#endif
	EXTERN__ void OnLoad(void);
	EXTERN__ void OnUnload(void);
	EXTERN__ void OnRelease_Before(void);
	EXTERN__ void OnRelease_After(ULONG count);
	EXTERN__ void OnBeginScene_Before(void);
	EXTERN__ void OnEndScene_Before(void);
	EXTERN__ void OnBeginScene_After(HRESULT result);
	EXTERN__ void OnEndScene_After(HRESULT result);
	EXTERN__ void OnReset_Before(D3DPRESENT_PARAMETERS* &pPresentationParameters);
	EXTERN__ void OnReset_After(D3DPRESENT_PARAMETERS* &pPresentationParameters, HRESULT result);
	EXTERN__ void OnPresent_Before(CONST RECT* &pSourceRect,CONST RECT* &pDestRect,HWND &hDestWindowOverride,CONST RGNDATA* &pDirtyRegion);
	EXTERN__ void OnPresent_After(CONST RECT* &pSourceRect,CONST RECT* &pDestRect,HWND &hDestWindowOverride,CONST RGNDATA* &pDirtyRegion, HRESULT &result);
};