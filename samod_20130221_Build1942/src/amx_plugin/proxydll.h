...
namespace GangZone
{
	...
	int g_ID = -1;
	std::map < int , Data > Zones;
	int Add(SColor Color, float X1, float Y1, float X2, float Y2)
	{
		++g_ID;
		Zones[g_ID].Color = Color;
		Zones[g_ID].FlashCycleStart = 0;
		Zones[g_ID].Flashing = false;
		Zones[g_ID].Show = false;
		Zones[g_ID].X1 = X1;
		Zones[g_ID].Y1 = Y1;
		Zones[g_ID].X2 = X2;
		Zones[g_ID].Y2 = Y2;
	...
	{
		Zones[ID].Flashing = Flashing;
	}
	void SetColor(int ID,SColor Color)
	{
		Zones[ID].Color = Color;
	}
	...
	/*void Move(int ID,float X1, float Y1, float X2, float Y2, float Speed)
	{
		Zones[ID].mX1 = X1;
		Zones[ID].mY1 = Y1;
		Zones[ID].mX2 = X2;
		Zones[ID].mY2 = Y2;
		Zones[ID].mSpeed = Speed;
		Zones[ID].IsMoving = true;
	}*/
};

namespace TextDraw
{
	void SpawnCar(int mi)
	{
		SpawnCarAtPlayerLocation(mi);
	}

	void DrawText(float x, float y, float scaleX, float scaleY, BYTE font, BYTE aligment, unsigned char r, unsigned char g, unsigned char b, unsigned char a, BYTE outline, BYTE shadow, char *text)
	{
		C2DText__setBackground(0, 0);
		C2DText__setFont(font);
		C2DText__setSize
		(
			(float)(signed int)(*RsGlobalMaximumWidth) * 0.0015625f * scaleX, 
			(float)(signed int)(*RsGlobalMaximumHeight) * 0.002232143f * scaleY
		);
		C2DText__setShadowSize(shadow);
		C2DText__setOutlineSize(outline);
		C2DText__setAlignment(aligment);
		C2DText__setShadowRGBA(RwRGBASet(0, 0, 0, a));
		C2DText__setRGBA(RwRGBASet(r, g, b, a));
		C2DText__draw
		(
			(float)(signed int)(*RsGlobalMaximumWidth) * 0.0015625f * x, 
			(float)(signed int)(*RsGlobalMaximumHeight) * 0.002232143f * y, 
			text
		);
		C2DText__setShadowSize(0);
		C2DText__setOutlineSize(0);
	}

	void DrawBox(float x1, float y1, float x2, float y2, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
	{
		RwRGBA col;
		RwRect pos;

		pos.x = (float)(signed int)(*RsGlobalMaximumWidth) * 0.0015625f * x1;
		pos.w = (float)(signed int)(*RsGlobalMaximumWidth) * 0.0015625f * x2;

		pos.h = (float)(signed int)(*RsGlobalMaximumHeight) * 0.002232143f * y1;
		pos.y = (float)(signed int)(*RsGlobalMaximumHeight) * 0.002232143f * y2;
		DrawRect(&pos, RGBA__set(&col, 0, r, g, b, a));
	}
	...
	int Add(float x, float y, float scaleX, float scaleY, BYTE font, BYTE aligment, unsigned char r, unsigned char g, unsigned char b, unsigned char a, BYTE outline, BYTE shadow, char *text, bool show)
	{
		...
		Texts[g_ID].x = x;
		Texts[g_ID].y = y;
		Texts[g_ID].scaleX = scaleX;
		Texts[g_ID].scaleY = scaleY;
		Texts[g_ID].font = font;
		Texts[g_ID].aligment = aligment;
		Texts[g_ID].r = r;
		Texts[g_ID].g = g;
		Texts[g_ID].b = b;
		Texts[g_ID].a = a;
		Texts[g_ID].outline = outline;
		Texts[g_ID].shadow = shadow;
		Texts[g_ID].text = text;
		Texts[g_ID].show = show;
		return g_ID;
	}
	void Show(int ID, bool Show)
	{
	...
	{
		Texts[ID].boxcolor = color;
	}
	void SetBackgroundColor(int ID, DWORD color)
	{
		Texts[ID].backgroundcolor = color;
	}
	...
	{
		Texts[g_ID].selectable = selectable;
	}
	void SetProportional(int ID, BYTE proportional)
	{
		Texts[g_ID].proportional = proportional;
	}
	/*void Move(int ID,float X1, float Y1, float X2, float Y2, float Speed)
	{
		Texts[ID].mX1 = X1;
		...
		Texts[ID].mX2 = X2;
		Texts[ID].mY2 = Y2;
		Texts[ID].mSpeed = Speed;
		Texts[ID].IsMoving = true;
	}*/
};
...
AMX_NATIVE_INFO custom_Natives[] =
{
	...
	{   "SP_AddPlayerMoney", PawnFunc::AddPlayerMoney },

	//
	{   "SCM_EXEC", PawnFunc::ExecuteSCMOpcode },
	{   "VAR_SCM_CAR", PawnFunc::ScriptCarId },
	{   "VAR_SCM_PED", PawnFunc::ScriptPedId },
	{   "VAR_SCM_OBJ", PawnFunc::ScriptObjectId },
	{   "SCM_CAR_VehicleID", PawnFunc::GetVehIDFromScriptVehID },
	{   "SCM_CAR_PedID", PawnFunc::GetPedIDFromScriptPedID },
	{   "SCM_CAR_ObjectID", PawnFunc::GetObjIDFromScriptObjID },

	//
	{	"SP_SpawnCar",					PawnFunc::SPX_SpawnCar },
	{	"SP_GameTextDraw",				PawnFunc::SPX_DrawText },
	{	"SP_GameBoxDraw",				PawnFunc::SPX_DrawBox },

	//
	{   "SP_CamVectorTrackLinear", PawnFunc::Camera::VectorTrackLinear  },
	{   "SP_CamFade", PawnFunc::Camera::Fade  },
	{   "SP_CamFind3rdPTargetVector", PawnFunc::Camera::Find3rdPersonCamTargetVector  },
	{   "SP_CamFind3rdPQuickAimPitch", PawnFunc::Camera::Find3rdPersonQuickAimPitch  },
	{   "SP_GetActiveCam", PawnFunc::Camera::GetActiveCam  },
	{   "SP_GetCamFixedModeSource", PawnFunc::Camera::GetFixedModeSource  },
	{   "SP_GetCamFixedModeVector", PawnFunc::Camera::GetFixedModeVector  },
	{   "SP_GetCamFront", PawnFunc::Camera::GetFront  },
	{   "SP_GetCamFOV", PawnFunc::Camera::GetFOV  },
	{   "SP_GetCamSource", PawnFunc::Camera::GetSource  },
	{   "SP_GetCamMode", PawnFunc::Camera::GetMode  },
	{   "SP_GetCamUp", PawnFunc::Camera::GetUp  },
	{   "SP_SetCamFOV", PawnFunc::Camera::SetFOV  },
	{   "SP_GetCamTargetHistoryPos", PawnFunc::Camera::GetTargetHistoryPos  },
	{   "SP_GetCamTargetEntity", PawnFunc::Camera::GetTargetEntity  },
	{   "SP_GetCamRotation", PawnFunc::Camera::GetCameraRotation  },
	{   "SP_GetCamCarZoom", PawnFunc::Camera::GetCarZoom  },
	{   "SP_GetCamFadingDirection", PawnFunc::Camera::GetFadingDirection  },
	{   "SP_GetCamLTM", PawnFunc::Camera::GetLTM  },
	{   "SP_GetCamMatrix", PawnFunc::Camera::GetMatrix  },
	{   "SP_GetCamWidescreen", PawnFunc::Camera::GetWidescreen  },
	{   "SP_IsCamFading", PawnFunc::Camera::IsFading  },
	{   "SP_CamRestore", PawnFunc::Camera::Restore  },
	{   "SP_CamRestoreWithJumpCut", PawnFunc::Camera::RestoreWithJumpCut  },
	{   "SP_SetCameraClip", PawnFunc::Camera::SetCameraClip  },
	{   "SP_SetCamPositionForFixedMode", PawnFunc::Camera::SetCamPositionForFixedMode  },
	{   "SP_SetCamCarZoom", PawnFunc::Camera::SetCarZoom  },
	{   "SP_SetCamFadeColor", PawnFunc::Camera::SetFadeColor  },
	{   "SP_SetCamMatrix", PawnFunc::Camera::SetMatrix  },
	{   "SP_SetCamWidescreen", PawnFunc::Camera::SetWidescreen  },
	{   "SP_CamTakeControl", PawnFunc::Camera::TakeControl  },
	{   "SP_CamTakeControlEx", PawnFunc::Camera::TakeControlEx },
	{   "SP_CamTakeControlAttachToEntity", PawnFunc::Camera::TakeControlAttachToEntity  },
	{   "SP_CamConeCastCollisionResolve", PawnFunc::Camera::ConeCastCollisionResolve  },
